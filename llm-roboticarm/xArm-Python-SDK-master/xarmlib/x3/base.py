...
class Base(BaseObject, Events):
    ...
    def _wait_feedback(self, timeout=None, trans_id=-1, ignore_log=False):
        ...
        while timeout is None or time.monotonic() < expired:
            if not self.connected:
                ...
                if not ignore_log:
                    self.log_api_info('wait_feedback, xarmlib is disconnect', code=APIState.NOT_CONNECTED)
                return APIState.NOT_CONNECTED, -1
            if self.error_code != 0:
                self._fb_transid_result_map.clear()
                if not ignore_log:
                    self.log_api_info('wait_feedback, xarmlib has error, error={}'.format(self.error_code), code=APIState.HAS_ERROR)
            ...
            if state >= 4:
                ...
                if state != 5 or state5_cnt >= 20:
                    self._fb_transid_result_map.clear()
                    if not ignore_log:
                        self.log_api_info('wait_feedback, xarmlib is stop, state={}'.format(state), code=APIState.EMERGENCY_STOP)
    ...
    def wait_move(self, timeout=None, trans_id=-1):
        ...
        while timeout is None or time.monotonic() < expired:
            if not self.connected:
                self.log_api_info('wait_move, xarmlib is disconnect', code=APIState.NOT_CONNECTED)
                return APIState.NOT_CONNECTED
            if self.error_code != 0:
                self.log_api_info('wait_move, xarmlib has error, error={}'.format(self.error_code), code=APIState.HAS_ERROR)
            ...
            if state >= 4:
                ...
                if state == 5:
                    state5_cnt += 1
                if state != 5 or state5_cnt >= 20:
                    self.log_api_info('wait_move, xarmlib is stop, state={}'.format(state), code=APIState.EMERGENCY_STOP)
